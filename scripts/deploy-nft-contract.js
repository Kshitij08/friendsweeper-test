...
async function main() {
  console.log("Deploying FriendsweeperNFT contract...");
  ...
  if (hre.network.name !== "hardhat" && hre.network.name !== "localhost") {
    ...
    await friendsweeperNFT.deployTransaction.wait(6);
    
    try {
      await hre.run("verify:verify", {
        address: address,
        constructorArguments: ["Friendsweeper NFT", "FSWEEP"],
      });
